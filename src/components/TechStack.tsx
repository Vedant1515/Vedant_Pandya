...
const TechStack = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (workElement) {
        ...
        setIsActive(scrollY > 500); // Fallback threshold if not found yet
      }
    };
      ...
      if (skill.icon) {
        ...
        if (ctx) {
          ...
          if (skill.name.length > 8) {
            ...
            if (words.length > 1) {
              ctx.fillText(words[0], 256, 210);
              ctx.fillText(words.slice(1).join(" "), 256, 300);
            } else {
  ...
  const spheres = useMemo(() => {
    return [...Array(skillData.length)].map((_, i) => ({
      scale: [0.8, 1, 0.9, 1.1][Math.floor(Math.random() * 4)],
